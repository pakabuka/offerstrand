...
# Using data only from 2019 to 2023
data <- subset(comp, cycle_id >= 19 & (simple_status == "accepted"
...
get_formula <- function(inputdata){
 ...
 # urm <- inputdata$urm_binary
  ...
  m5 <- glm(status_binary ~ lsat + gpa + softs_binary, data = dataTrain, family = binomial(link = "logit"))
  m6 <- glm(status_binary ~ lsat + gpa + softs_binary + inter_binary, data = dataTrain, family = binomial(link = "logit"))
  m7 <- glm(status_binary ~ lsat + gpa + softs_binary + urm_binary, data = dataTrain, family = binomial(link = "logit"))
